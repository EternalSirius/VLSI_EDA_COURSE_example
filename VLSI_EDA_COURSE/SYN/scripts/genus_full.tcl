...
set _OUTPUTS_PATH ./outputs_full
set _REPORTS_PATH ./reports_full
set _LOG_PATH ./logs_full
...
set_attribute lib_search_path {/eda/PDK_DIR/TSMC/180_DIG_CELL/TSMCHOME/digital/Front_End/timing_power_noise/NLDM/tcb018g3d3_280a/}
set_attribute library [list tcb018g3d3tc.lib ]
set_attribute lef_library /eda/PDK_DIR/TSMC/180_DIG_CELL/TSMCHOME/digital/Back_End/lef/tcb018g3d3_280a/lef/tcb018g3d3_6lm.lef
...
#HS 3
define_dft test_clock -name scanclk -period $myPeriod_ps [clock_ports]
...
#HS
set_attribute lp_clock_gating_test_signal tm "/designs/$DESIGN"
...
if {$numScanChains != "0" || $numScanChains != ""} {
    for {set i 1} {$i <= $numScanChains} {incr i} {
	puts "define_scan_chain -name chain$i -sdi sdi_$i -sdo sdo_$i -create_ports"
	define_scan_chain -name chain$i -sdi sdi_$i -sdo sdo_$i -create_ports
    }
}
